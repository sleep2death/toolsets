...
export default function profile(app, rds) {
  ...
  app.post("/api/profile/save", checkJWT, async (req, res) => {
    try {
      ...
      // Update user email
      const oEmail = await rds.HGET(`uuid:${req.jwt.uuid}`, "email");
      if (oEmail !== req.body.email) {
        ...
        await rds
          .multi()
          .SET(`email:${req.body.email}`, req.jwt.uuid) // set new email for this uuid
          .DEL(`email:${oEmail}`) // delete old one
          .HSET(`uuid:${req.jwt.uuid}`, "email", req.body.email) // set email in hashmap
          .exec();
      }

      await rds
        .multi()
        .HSET(`uuid:${req.jwt.uuid}`, "nickname", req.body.nickname)
        .HSET(`uuid:${req.jwt.uuid}`, "avatar_seed", req.body.seed)
        .exec();
